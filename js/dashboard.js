...
const translations = {
    ka: {
        header: 'საზოგადოებრივი ტრანსპორტი თბილისში',
        loading: 'იტვირთება...',
        noData: 'მონაცემები ხელმისაწვდომი არ არის',
        bus: 'ავტობუსი',
        metro: 'მეტრო',
        ...
        weekend: 'შაბ-კვ.',
        holiday: 'უქმე დღე',
        totalPassengers: 'სულ მგზავრობა',
        vsPrevDay: 'vs წინა დღე',
        lastUpdate: 'ბოლო განახლება:',
        methodology: 'მეთოდოლოგია',
        downloadData: 'მონაცემები',
        share: 'გაზიარება:',
        shareFacebook: 'გაზიარება Facebook-ზე',
        shareLinkedIn: 'გაზიარება LinkedIn-ზე',
        shareEmail: 'გაზიარება ელფოსტით',
        modalTitle: 'მეთოდოლოგია',
        modalSource: 'მონაცემთა წყარო:',
        modalSourceText: 'მგზავრობის ყოველდღიური მონაცემები ეყრდნობა',
        modalSourceLink: 'თბილისის სატრანსპორტო კომპანიის',
        modalSourceText2: 'ოფიციალურ ინფორმაციას.',
        modalCoverage: 'მახასიათებლები:',
        modalCoverageText: 'მონაცემები ფარავს საზოგადოებრივი ტრანსპორტის ოთხ სახეობას - ავტობუსს, მეტროს, მიკროავტობუსსა და საბაგიროს. პლატფორმაზე მოცემული უახლესი მონაცემი წინა დღის მაჩვენებელია.',
        ...
        modalCitationText: 'მონაცემების გადმოწერის და გამოყენების შემთხვევაში, გთხოვთ, წყარო მიუთითოთ შესაბამის ფორმატში - <i>თბილისის საზოგადოებრივი ტრანსპორტის ყოველდღიური დინამიკა - თბილისის სატრანსპორტო კომპანია, Z.axis (2026).</i>'
    },
    en: {
        ...
        modalTitle: 'Methodology',
        modalSource: 'Data Source:',
        modalSourceText: 'Daily passenger counts are based on official information from',
        modalSourceLink: 'Tbilisi Transport Company',
        modalSourceText2: '',
        modalCoverage: 'Coverage:',
        modalCoverageText: 'The data includes all four public transport modes in Tbilisi: buses, metro, minibuses (marshrutkas), and cable cars. The latest data shown on the platform represents the previous day\'s ridership.',
...
function updateTotalCard(total, change, date) {
    ...
    if (changeElement) {
        if (change !== null) {
            ...
            changeElement.innerHTML = `${changeSymbol} ${Math.abs(change)}% ${t('vsPrevDay')}`;
        } else {
...
function createInsights(data) {
    if (data.length < 2) return `<p>${t('noData')}</p>`;
        ...
        return `
            <div class="insight-card">
                ...
                <div class="insight-change ${mode}-change ${changeClass}" style="color: ${changeClass === 'change-positive' ? '#10b981' : changeClass === 'change-negative' ? '#ef4444' : '#6b7280'};">
                    ${changeSymbol} ${Math.abs(change)}% ${t('vsPrevDay')}
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div class="insight-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; position: relative;">
            <h3 style="color: rgba(255,255,255,0.9);">${t('totalPassengers')}</h3>
            ...
            <div class="insight-change total-change" style="color: rgba(255,255,255,0.9);">
                ${getChangeSymbol(totalChange)} ${Math.abs(totalChange)}% ${t('vsPrevDay')}
            </div>
        </div>
        ${cards}
    `;
...
async function initDashboard() {
    ...
    if (data.length === 0) {
        document.getElementById('dashboard').innerHTML = `<p class="loading">${t('noData')}</p>`;
        return;
    }
    ...
    const html = `
        <div class="language-toggle">
            <button class="lang-btn ${currentLanguage === 'ka' ? 'active' : ''}" onclick="toggleLanguage()">
                ${currentLanguage === 'ka' ? 'EN' : 'ქარ'}
            </button>
        </div>
        ...
        <div class="modal" id="methodModal" onclick="if(event.target === this || event.target.classList.contains('modal-backdrop')) closeMethodModal()">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>${t('modalSource')}</strong> ${t('modalSourceText')} <a href="https://ttc.com.ge" target="_blank" rel="noopener">${t('modalSourceLink')}</a>${t('modalSourceText2') ? ' ' + t('modalSourceText2') : '.'}</p>
                    <p><strong>${t('modalCoverage')}</strong> ${t('modalCoverageText')}</p>
                    <p><strong>${t('modalHolidays')}</strong> ${t('modalHolidaysText')}</p>
                    <p><strong>${t('modalCitation')}</strong> ${t('modalCitationText')}</p>
                </div>
            </div>
        </div>
        ...
        <div class="footer">
            <div class="footer-top">
                <div class="last-update">
                    <span class="last-update-label">${t('lastUpdate')}</span>
                ...
                <div class="social-share">
                    ...
                    <button class="share-button linkedin" onclick="shareOnLinkedIn()" title="${t('shareLinkedIn')}">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                        </svg>
                    </button>
                    <button class="share-button email" onclick="shareViaEmail()" title="${t('shareEmail')}">
